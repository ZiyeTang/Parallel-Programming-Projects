...
void MNIST::read_mnist_label(std::string filename, Matrix& labels, int batch_size) {
  std::ifstream file(filename, std::ios::binary);
  // std::cout<<"Reading: "<<filename<<std::endl;
  if (file.is_open()) {
    int magic_number = 0;
...
void MNIST::read() {
  read_mnist_data(data_dir + "train-86-images-idx3-ubyte", train_data);
  read_mnist_data(data_dir + "t10k-86-images-idx3-ubyte", test_data);
  read_mnist_label(data_dir + "train-86-labels-idx1-ubyte", train_labels);
  read_mnist_label(data_dir + "t10k-86-labels-idx1-ubyte", test_labels);
}
