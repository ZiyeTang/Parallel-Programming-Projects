...
__global__ void conv_forward_kernel(float *output, const float  *input, const float  *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    ...
    //             for (int q = 0; q < K; q+=2) {
    //                 if (h+p < Height && w+q < Width) {
    //                     if (w+q+1<Width && q+1<K) {
    //                         acc = __hadd2(acc, __hmul2( __floats2half2_rn(in_4d(b, c, h + p, w + q), in_4d(b, c, h + p, w + q+1)),  __floats2half2_rn(mask_4d(m, c, p, q), mask_4d(m, c, p, q+1))));
    //                         continue;
    //                     }
    //                     acc = __hadd2(acc, __hmul2( __floats2half2_rn(in_4d(b, c, h + p, w + q), 0),  __floats2half2_rn(mask_4d(m, c, p, q), 0)));
    //                 }
    //             }
    //         }
    //     }
    //     out_4d(b, m, h, w) =  __half2float(__low2half(acc)) + __half2float(__high2half(acc));
    // }
